...
function validateInput(objectToBeValidateStr, schema) {
    return function (req, res, next) {
        objectToBeValidate = objectToBeValidateStr === "params" ? req.params : req.body;
...
function getAnnouncements(req, res) {
    functions.formatQuery(req.query).then(function (formatedQuery) {
        database.Announcements.find(formatedQuery.filters).select(formatedQuery.fields).populate('_about', 'name public').populate('attachments', 'name').sort(formatedQuery.sort).exec(function (err, announcements) {
            if (err) {
...
function getAnnouncement(req, res) {
    ...
    database.Announcements.findOne({_id: announcementsId}).populate('_about', 'name public').populate('attachments', 'name').exec(function (err, announcement) {
        if (err) {
...
function getAnnouncementsFeed(req, res) {
    ...
    database.AnnouncementsCategories.find(filter).select('_id name').exec(function (err, rssCategories) {
        if (err || !rssCategories.length) {
            ...
            database.Announcements.find({_about: {$in: rssCategories}}).populate('_about', 'name').populate('attachments', 'name').exec(function (err, announcements) {
                if (err) {
                    res.status(500).json({message: 'Συνεβη καποιο λάθος κατα την λήψη ανακοινώσεων'});
                } else {
                    announcementsFunctions.getAnnouncementsRSSPromise(announcements, rssCategories, req.params.categoryIds, feedType, res, login).then(function (response) {
                        res.send(response);
                    }).catch(function (err) {
